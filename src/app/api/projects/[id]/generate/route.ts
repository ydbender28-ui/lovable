...
export async function POST(req: Request, ctx: RouteContext<"/api/projects/[id]/generate">) {
  ...
  const [finalRoute] = await Promise.all([
    hasExisting
      ...
      : Promise.resolve({ intent: prompt.slice(0, 80), taskType: "new-build" as const, model: { model: "gemini-2.5-flash", displayName: "Gemini 2.5 Flash", provider: "google" as const, maxTokens: 16000, costPer1kInput: 0.00015, costPer1kOutput: 0.0035 }, modelReason: "New build" }),
