...
export default function ProjectWorkspace({
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0a0a0f]">
      ...
      <header className="border-b border-white/10 bg-[#0a0a0f]/90 backdrop-blur px-4 py-2.5 flex items-center justify-between shrink-0 gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className={`text-xs rounded-lg px-2.5 py-1.5 border transition-colors ${showEnvPanel ? "border-fuchsia-400/50 bg-fuchsia-500/10 text-fuchsia-300" : "border-white/10 bg-white/5 text-gray-300 hover:bg-white/10"}`}
          >
            {"{}"} <span className="hidden sm:inline">APIs</span>
            {Object.keys(envVars).length > 0 && (
              <span className="ml-1 rounded-full bg-fuchsia-500/30 text-fuchsia-300 px-1.5 text-[10px]">{Object.keys(envVars).length}</span>
            )}
          </button>
          ...
          {publishUrl ? (
            <div className="flex items-center gap-1.5">
              ...
              <button onClick={() => navigator.clipboard.writeText(publishUrl ?? "")}
                className="text-xs rounded-lg border border-white/10 bg-white/5 text-gray-300 px-2 py-1.5 hover:bg-white/10 hidden sm:block">
                Copy
              </button>
            ...
            >
              {publishing ? (
                <><span className="h-1.5 w-1.5 rounded-full bg-fuchsia-400 animate-pulse" /> Publishing...</>
              ) : "Publish"}
            </button>
          )}
      ...
      {/* Env panel */}
      {showEnvPanel && (
        <div className="border-b border-white/10 bg-[#0d0d14] px-4 py-3 shrink-0">
          <p className="text-xs text-gray-400 mb-3">
            API keys injected into generated code as constants. Not exposed via publish.
          </p>
          <div className="flex flex-wrap gap-2 mb-3">
            {Object.entries(envVars).map(([k, v]) => (
              <div key={k} className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1 text-xs">
                ...
                <button onClick={() => removeEnvVar(k)} className="text-gray-600 hover:text-red-400 ml-1 transition-colors">×</button>
              </div>
            ))}
          </div>
          <div className="flex gap-2">
            <input value={newKey} onChange={(e) => setNewKey(e.target.value)} placeholder="KEY"
              className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white placeholder:text-gray-600 focus:outline-none focus:border-fuchsia-400/40 font-mono w-36" />
            ...
            <button onClick={saveEnvVar} disabled={!newKey.trim()}
      ...
      <div className="hidden sm:flex flex-1 overflow-hidden">
        <div className="w-[340px] flex flex-col border-r border-white/10 shrink-0">
          {chatPanel}
        </div>
      ...
      {/* ── Mobile layout: tab switcher at bottom ── */}
        ...
        <div className="shrink-0 border-t border-white/10 bg-[#0c0c12] flex">
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium transition-colors flex flex-col items-center gap-0.5 ${
              mobileTab === "chat" ? "text-fuchsia-400 border-t-2 border-fuchsia-400 -mt-px" : "text-gray-500"
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium transition-colors flex flex-col items-center gap-0.5 ${
              mobileTab === "preview" ? "text-fuchsia-400 border-t-2 border-fuchsia-400 -mt-px" : "text-gray-500"
            }`}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
