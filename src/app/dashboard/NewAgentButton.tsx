            ...
            {step === "template" ? (
              <div className="p-6">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  {TEMPLATES.map(t => (
                    <button key={t.name} onClick={() => useTemplate(t)}
                      className="text-left rounded-xl border border-[#ececf1] bg-white p-4 hover:border-indigo-400/40 hover:bg-indigo-500/5 transition-colors">
                ...
                <button onClick={() => setStep("custom")}
                  className="w-full rounded-xl border border-dashed border-[#ececf1] bg-transparent text-gray-500 py-3 text-sm hover:text-gray-300 hover:border-white/20 transition-colors">
                  Start from scratch →
                </button>
              </div>
            ) : (
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Support Bot"
                    className="w-full rounded-xl border border-[#ececf1] bg-white/5 px-3 py-2.5 text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-indigo-400/40" />
                </div>
                ...
                <div>
                  ...
                  <textarea value={systemPrompt} onChange={e => setSystemPrompt(e.target.value)} rows={6}
                    ...
                    className="w-full rounded-xl border border-[#ececf1] bg-white/5 px-3 py-2.5 text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-indigo-400/40 resize-none font-mono leading-relaxed" />
