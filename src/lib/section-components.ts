...
export default function CTA({ title, subtitle, cta, image }: { title: string; subtitle?: string; cta: { text: string; href?: string }; image?: string }) {
  ...
  useEffect(() => { const h = () => setIsMobile(window.innerWidth < 768); window.addEventListener('resize', h); return () => window.removeEventListener('resize', h); }, []);
  return (
    <section ref={ref as any} style={{ position:'relative', padding: isMobile ? '64px 20px' : '100px 40px', textAlign:'center', overflow:'hidden', background: image ? 'none' : 'var(--bg,#fff)', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(32px)', transition: 'opacity 0.6s ease, transform 0.6s ease' }}>
      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:700, margin:'0 auto' }}>
        <h2 style={{ fontSize:'clamp(32px,5vw,56px)', fontWeight:800, lineHeight:1.1, color: image ? '#fff' : 'var(--fg,#111)', letterSpacing:'-0.02em' }}>{title}</h2>
...
- CRITICAL: ONLY import from this exact list. NEVER invent component names or paths like CartDrawer, PizzaBuilder, OrderTracker, ProductCard, HeroSection, AdminPanel, Dashboard, etc. — and this applies to EVERY import path, not just /components/sections/. Importing from ANY path not listed above (/components/AdminPanel, /components/Dashboard, /lib/whatever, or anything else you make up) causes the exact same fatal "Could not find module" crash. If a requested feature (admin panel, dashboard, custom form, etc.) has no matching pre-built component, write it as a plain function DIRECTLY INSIDE /App.tsx — never as a separate imported file. When in doubt, inline it in App.tsx.`;
